...
describe('🧲 9. Captura Silenciosa de Leads (Carrinho Abandonado)', () => {
  test('Retorna status "success" mesmo se a agenda do dia estiver completamente lotada', () => {
    const dayEvents = [
      {
        summary: 'Festa: Cliente 1',
        description: 'Serviços: Buffet Premium',
        start: { dateTime: '2026-10-10T14:00:00Z' },
        end: { dateTime: '2026-10-10T18:00:00Z' }
      },
      {
        summary: 'Festa: Cliente 2',
        description: 'Serviços: Buffet Essencial',
        start: { dateTime: '2026-10-10T14:00:00Z' },
        end: { dateTime: '2026-10-10T18:00:00Z' }
      },
      {
        summary: 'Locação: Cliente 3',
        description: 'Serviços: Hot Dog Gourmet, Carrinho Pipoca',
        start: { dateTime: '2026-10-10T14:00:00Z' },
        end: { dateTime: '2026-10-10T18:00:00Z' }
      },
      {
        summary: 'Locação: Cliente 4',
        description: 'Serviços: Cama Elástica, FestBar Drinks',
        start: { dateTime: '2026-10-10T14:00:00Z' },
        end: { dateTime: '2026-10-10T18:00:00Z' }
      }
    ];
    ...
    const leadRequest = {
      action: 'lead_capture',
      clientName: 'Lead Abandonado Teste',
      eventLocation: 'Águas Claras',
      guests: '50',
      total: 'R$ 2.500,00'
    };

    const result = validateAppointment(dayEvents, leadRequest);
    expect(result).toEqual({ status: 'success' });
  });

  test('Não exige serviços válidos ou dados estritos de agendamento final', () => {
    const dayEvents = [];
    const leadIncompleto = {
      action: 'lead_capture',
      services: '',
      clientName: 'Lead Sem Serviços Selecionados'
    };

    const result = validateAppointment(dayEvents, leadIncompleto);
